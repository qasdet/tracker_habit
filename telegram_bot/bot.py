...
def start(update, context):
    ...
    try:
        ...
        update.message.reply_text(
            f"С возвращением, {telegram_user.user.username}! Вы будете получать уведомления о ваших привычках."
        )
    except TelegramUser.DoesNotExist:
        update.message.reply_text(
            "Пожалуйста, свяжите свой аккаунт через веб-интерфейс."
        )
...
def send_reminder(user, habit):
    try:
        ...
        try:
            ...
            print(
                f"Successfully sent reminder to user {user.username} for habit {habit.name}"
            )
        except Exception as e:
            print(
                f"Error sending message to Telegram: {str(e)}\nUser: {user.username}\nTelegram ID: {telegram_user.telegram_id}"
            )
    except TelegramUser.DoesNotExist:
